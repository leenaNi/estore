...
use App\Models\ProductWiseInwardTransaction;
...
class DistributorOrdersController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach($orders as $orderKey => $order) {
            $hasProds = HasProducts::where('order_id', $order->id)->get();
            $categories = [];
            foreach($hasProds as $hasProdKey => $hasProd) {
                $prodCategories = DB::table('has_categories')->where('prod_id', $hasProd->prod_id)->first(['cat_id']);
                if($prodCategories){
                    array_push($categories, $prodCategories->cat_id);
                }
            }
            $categories = array_unique($categories);
            $order->categories = DB::table('store_categories')->join('categories', 'categories.id', '=', 'store_categories.category_id')
    ...
    public function add()
    {
        ...
        return view(Config('constants.adminDistributorOrderView') . '.addEdit', compact('order', 'products', 'action'));
    }
    ...
    public function edit()
    {
        ...
        $courierId = HasCourier::where('status', 1)->where('store_id', $order->store_id)->pluck("courier_id");
        ...
        if ($order->prefix == Helper::getSettings()['prefix']) {
            ...
            $products = HasProducts::where("order_status", "!=", 0)->where("order_id", Input::get('id'))->where('prefix', $order->prefix)->where('store_id', $order->store_id)->first();
        ...
        if (!empty(Input::get('cartdata'))) {
            ...
            foreach ($existingcart as $cart) {
                ...
                if (!empty($cart->options->sub_prod)) {
                    $prd = DistributorProduct::find($cart->options->sub_prod);
                    $prd->stock = $prd->stock + $cart->qty;
                    $prd->update();
                    $prod = DistributorProduct::find($cart->options->sub_prod);
                } else if (!empty($cart->options->combos)) {
                    foreach ($cart->options->combos as $key => $val) {
                        if (isset($val->sub_prod)) {
                            $prd = DistributorProduct::find($val->sub_prod);
                            $prd->stock = $prd->stock + $cart->qty;
                            $prd->update();
                        } else {
                            $prd = DistributorProduct::find($key);
                            $prd->stock = $prd->stock + $cart->qty;
                            $prd->update();
                        }
                    }
    ...
    public function updateRetutnQty()
    {
        ...
        foreach (Input::get("pid") as $key => $val) {
            if (Input::get("qty_returned")[$key]) {
                ...
                $mainProd = DistributorProduct::find($val);
        ...
        if ($order->order_status == 1) {
            ...
            foreach ($cartContent as $cart) {
                ...
                if ($product->prod_type == 1) {
                    ...
                    $Subproduct = DistributorProduct::find($subProd);
    ...
    public function invoice($id = null)
    {
        ...
        foreach ($orders as $key => $order) {
            ...
            foreach ($catlogs as $key2 => $product) {
                ...
                // dd($product['id']);
                $catlogs[$key2]['product'] = DistributorProduct::where('id', $product['id'])->first();
    ...
    public function order_return()
    {
        ...
        //                 $q->with('user');
        //             }])->where("store_id", $order->store_id)->orderBy('id', 'desc')->get()->toArray();
        // return view(Config('constants.adminOrderView') . '.returnOrders', compact('return'));
        $return = ReturnOrder::with('reason', 'opened', 'product_id', 'order_id', 'return_status_id')->with(['order_id' => function ($q) {
            $q->with('user');
        }])->where("store_id", $order->store_id)->orderBy('id', 'desc');
    ...
    public function update_return_order_status()
    {
        ...
        if (Input::get('return_status') == 2) {
            ...
            $product = DistributorProduct::find($returnorder->product_id);
            if (count($checkStockEnabled) != 0 && $product->is_stock == 1):
                $product = DistributorProduct::find($returnorder->sub_prod);
                ...
                if (isset($returnorder->exchange_product_id) && $returnorder->exchange_product_id != 0):
                    $configProduct = DistributorProduct::find($returnorder->exchange_product_id);
    ...
    {
        $parent_prod = '';
        $product = DistributorProduct::where('id', Input::get('id'))->select('id', 'stock', 'price', 'spl_price', 'selling_price', 'prod_type', 'parent_prod_id')->get()->toArray();
        if ($product[0]['stock'] > 0) {
            if ($product[0]['parent_prod_id'] != 0) {
                $parent_product = DistributorProduct::where('id', $product[0]['parent_prod_id'])->select('id', 'price', 'prod_type')->get()->toArray();
    ...
    {
        $product = DistributorProduct::where('id', Input::get('id'))->select('id', 'price', 'stock', 'prod_type', 'parent_prod_id', 'spl_price', 'selling_price')->get()->toArray();
        if ($product[0]['stock'] < Input::get('stock')) {
            if ($product[0]['parent_prod_id'] != 0) {
                $parent_product = DistributorProduct::where('id', $product[0]['parent_prod_id'])->select('id', 'price')->get()->toArray();
                $price = $product[0]['price'] + $parent_product[0]['price'];
            } else {
                $price = $product[0]['spl_price'];
                if ($product[0]['spl_price'] == 0.00) {
    ...
    public function edit_re_order()
    {
        ...
        $product_all = DistributorProduct::select('id', 'product', 'parent_prod_id')->get()->toArray();
    ...
    public function save_re_order()
    {
        ...
        foreach (Input::get('old_product') as $prod) {
            ...
            $prods = DistributorProduct::find($prod_id);
        ...
        foreach (Cart::instance('adminshop')->content() as $order_session) {
            ...
            $prd = DistributorProduct::find($order_session->options->sub_prod);
    ...
    public function configProduct($prod_id, $quantity, $sub_prod)
    {
        if (Helper::checkStock($prod_id, $quantity, $sub_prod) == "In Stock") {
            $product = DistributorProduct::find($sub_prod);
            $product = DistributorProduct::find($prod_id);
            $cats = [];
            foreach ($product->categories as $cat) {
                array_push($cats, $cat->id);
    ...
    public function createOrder()
    {
        ...
        $viewname = Config('constants.adminDistributorOrderView') . '.create-order';
    ...
    public function getDistributor()
    {
        ...
        foreach ($result as $k => $res) {
            $data[$k]['id'] = $res->id;
            $data[$k]['store_name'] = $res->store_name;
            $tele = ($res->identity_code) ? " / " . $res->identity_code : '';
            $ele = ($res->email) ? " / " . $res->email : '';
            $data[$k]['label'] = $res->store_name . "" . $tele . "" . $ele;
            $data[$k]['firstname'] = $res->firstname;
            $data[$k]['identity_code'] = $res->identity_code;
            $data[$k]['phone_no'] = $res->phone_no;
    ...
    public function getCustomerData()
    {
        if (!empty(Input::get('user_id'))) {
            Session::put('distributor_store_id', Input::get('distributor_id'));
            $getStorePrefix = DB::table('stores')->where('id', Input::get('distributor_id'))->first();
            Session::put('distributor_store_prefix', $getStorePrefix->prefix);
            $user = User::where('id', Input::get('user_id'))->with('addresses')->first();
    ...
    public function getSearchProds()
    {
        ...
        $products = DB::table('products')->where('store_id', Session::get('distributor_store_id'))->where("is_individual", 1)->where('status', 1)->where('product', "like", "%" . $searchStr . "%")->orWhere('id', "like", "%" . $searchStr . "%")->get(['id', 'product']);
    ...
    public function getProdPrice()
    {
        ...
        if (Input::get('pprd') == 1) {
            ...
            $pprod = DistributorProduct::find(Input::get('subprdid'));
            $price = $pprod->price+@DistributorProduct::find($pprod->parent_prod_id)->selling_price;
    ...
    public function editOrderChkStock()
    {
        ...
        if ($ptype == 1 && $is_stockable->status == 1) {
            ...
            foreach ($spidarr as $combos) {
                $spid[DistributorProduct::find($combos)->parent_prod_id] = $combos;
            }
            $chk = Helper::checkStock($ppid, $qty, $spid);
        } else {
            $chk = "In Stock";
    ...
    public function getCartEditProdVar()
    {
        ...
        $prd = DistributorProduct::find($prdid);
    ...
    public function cartEditGetComboSelect()
    {
        ...
        foreach ($prd->comboproducts()->get() as $prdC) {
            ...
            if ($prdC->prod_type == 3) {
                $getSel .= "<br/>" . $prdC->product . "(" . $prdC->categories()->first()->category . ")";
                $getSubPrd = DistributorProduct::find($prdC->id)->subproducts()->get();
    ...
    public function saveOrderSuccess($paymentMethod, $paymentStatus, $payAmt, $trasactionId, $transactionStatus, $des = null, $transactioninfo = null)
    {
        ...
        $jsonString = Helper::getSettings();
        $order->prefix = Session::get('distributor_store_prefix');;
        $order->store_id = Session::get('distributor_store_id');
    ...
    public function getTotalQtyProductWise($orderId) // for validation of received qty total
    {
        $inwardTransactionResult = DB::table('inward_transaction')
        ->join('productwise_inward_transaction', 'inward_transaction.id', '=', 'productwise_inward_transaction.inward_transaction_id')
        ->where('inward_transaction.order_id', $orderId)
        ->orderBy('productwise_inward_transaction.has_product_id', 'asc')
        ->get();
        //echo "<pre>";print_r($inwardTransactionResult);exit;
        $totalQtyAndPriceArray = array();
        $tempProductId = 0;
        $totalQty = 0;
        
        foreach($inwardTransactionResult as $inwardTransactionData)
        {
            $hasProductId = $inwardTransactionData->has_product_id;
            //echo "<br>".$hasProductId." != ".$tempProductId;
            if($tempProductId > 0 && $hasProductId != $tempProductId)
            {
                $totalQtyAndPriceArray[$tempProductId] = $totalQty;
                $totalQty = 0;
            }
            $totalQty = $totalQty+$inwardTransactionData->qty;
            $tempProductId = $hasProductId;
        }
        $totalQtyAndPriceArray[$tempProductId] = $totalQty;
        //echo "<pre>";print_r($totalQtyAndPriceArray);exit;
    ...
    public function getProductForMapping() // from search product
    {
        if (!empty(Input::get('term'))) 
        {
            $term = Input::get('term');
            //echo "insdei if".Session::get('store_id');
            $productResult = DB::table('products')
            ->join("stores", "stores.id", "=", "products.store_id")
            ->where('store_id', Session::get('store_id'))
            ->where('product', 'LIKE',"%$term%")
            ->get(['products.id', 'products.product as product_name','stores.merchant_id']);
            //echo "<pre>";print_r($productResult);
            $data = [];
            $k = 0;
            foreach ($productResult as $productData) 
            {
                $data[$k]['id'] = $productData->id;
                $data[$k]['label'] = $productData->product_name;
                $data[$k]['name'] = $productData->product_name;
                $data[$k]['merchant_id'] = $productData->merchant_id;
                $k++;
            }
            echo json_encode($data);
        }
      
    } // Edn getProductForMapping()

    public function saveInwardData() // save inward data
    {
        $allinput = Input::all();
        ...
        {
            $productData = $allinput['data'][$i];
            $orderProductId = $productData['order_product_id'];
            $merchantId = $productData['m_id'];
            $distributorId = $productData['d_id'];
            $merchantProductId = $productData['m_product_id'];
            $distributorProductId = $productData['d_product_id'];
            $receivedQty = $productData['received_qty'];
            $unitPrice = $productData['unit_price'];
            $totalPrice = $productData['total_price'];
            $isMapped = $productData['is_mapped'];
            $totalReceivedQty = $totalReceivedQty + $receivedQty;
            $totalReceivedProductPrice = $totalReceivedProductPrice + $totalPrice;
            ...
            $productWiseInwardInsert[] = [
                'inward_transaction_id' => 0,
                'has_product_id' => $orderProductId,
                'order_type' => 'receive',
                'qty'=> $receivedQty,
        ...
        //echo "<pre>";print_r($productWiseInwardInsert);
       // echo "<pre>";print_r($insertData);
        ...
        if($isSuccess)
        {
            ...
            } // End lastInsertId check if
            else
            {
                echo false;
            }
        } // End isSuccess check if
        else
        {
            echo false;
        }
    ...
    public function getInwardTransaction() // inwward transaction list 
    {
        ...
        //echo "<pre>";print_r($inwardTransactionResult);exit;
        //$inwardTransactionResult = $inwardTransactionResult->paginate(Config('constants.paginateNo'));
        $viewname = Config('constants.adminDistributorOrderView') . '.inward-list';
        $data = ['inwardTransaction' => $inwardTransactionResult];
        
        return Helper::returnView($viewname, $data);

    } // End getInwardTransaction()

    public function getProductDiscrepency() // display discrepancy page
    {
        $orderId = Input::get('id');
        $totalQtyAndPriceArray = $this->getTotalQtyProductWise($orderId);
        ...
        /*$inwardTransactionResult = DB::table('productwise_inward_transaction')
            ->join("has_products", "productwise_inward_transaction.has_product_id", "=", "has_products.id")
            ->join("inward_transaction", "inward_transaction.id", "=", "productwise_inward_transaction.inward_transaction_id")
            ->where('inward_transaction.order_id', $orderId)
            ->get();*/
        //echo "<pre>";print_r($inwardTransactionResult);exit;
        $viewname = Config('constants.adminDistributorOrderView') . '.product-discrepency';
