...
//         dd($orders);courier
    ...
    public function settlementSummary() {
        $orders = DB::table("has_products")->orderBy("has_products.id", "desc")->join("stores", "stores.id", '=', "has_products.store_id")->
                ...
                ->join("orders", "orders.id", '=', "has_products.order_id")
//              
                ->select(DB::raw('sum(has_products.pay_amt) as totalOrder, (select sum(has_products.pay_amt) from orders where orders.courier = 0) as c3, (select sum(has_products.pay_amt) from orders where orders.courier = 1) as c4, stores.store_name, sum(payment_settlement.settled_amt) as totalPaid, sum(payment_settlement.order_amt) as orderAmt'))
