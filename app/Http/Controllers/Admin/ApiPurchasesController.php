...
class ApiPurchasesController extends Controller
{
    public function getDistributorOffers()
    {
        ...
        if (!empty(Input::get("merchantId")) && !empty(Input::get("distributorId"))) {
            ...
            if ($storeId && $storeId != null) {
                $offers = DB::table('offers')->where('status', 1)->where('store_id', $storeId->id)->get(['id', 'offer_name', 'offer_image', 'type', 'offer_discount_type', 'offer_type', 'can_combined']);
                if (count($offers) > 0) {
                    $isError = 0;
                } else {
                    $isError = 1;
    ...
    public function getAllOffers()
    {
        ...
        if (!empty(Input::get("merchantId"))) {
            ...
            if (count($storeIds) > 0) {
                $offers = DB::table('offers')->where('status', 1)->whereIn('store_id', $storeIds)->get(['id', 'offer_name', 'offer_image', 'type', 'offer_discount_type', 'offer_type', 'can_combined']);
                if (count($offers) > 0) {
                    $isError = 0;
                } else {
                    $isError = 1;
