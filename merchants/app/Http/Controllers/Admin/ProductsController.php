    ...
    public function update4()
    {
        ...
        if ($isExists) {
            ...
            foreach ($prods as $key => $prd) {
                // dd($prd["options"]);
                $newConfigProduct = Product::create(['product' => $prod->product . ' - Variant - ' . ($key + 1), 'is_avail' => 1, 'is_stock' => $prod->is_stock, 'parent_prod_id' => $prod->id, 'is_individual' => 0, 'prod_type' => 1, 'attr_set' => $prod->attr_set, 'price' => $prods[$key]['price'], 'stock' => $prods[$key]['stock'], 'is_avail' => $prods[$key]['is_avail'], 'store_id' => Session::get('store_id')]);
                //*
    ...
    public function product_import_csv1($path, $filename)
    {
        ...
        if (($handle = fopen($csv_file, "r")) !== false) {
            ...
            while (($data = fgetcsv($handle, 1000, ",")) !== false) {
                ...
                $meta_key = $col[22];
                $meta_desc = $col[23];
                $is_shipped_inter = $col[24];
                $is_referal_discount = $col[25];
                $tags = $col[26];
                $categories = $col[27];
                $related = $col[28];
                $upsell = $col[29];
                if (!empty($id)) {
                    ...
                    if (strtolower($id) == 'null') {
                        ...
                        if (!empty($prodT)) {
                            if (is_numeric($prodT) && ($prodT == 1 || $prodT == 3)) {
                        ...
                        if (!empty($attr_set)) {
                            ...
                            $atr = AttributeSet::where([strtolower('attr_set') => $attr_set])->first();
                            if (!empty($atr)) {
                                $attr_set = $atr->id;
                            } else {
                    ...
                    if (strtolower($id) != 'null') {
                        ...
                        if (!empty($attr_set)) {
                            $atr = AttributeSet::where([strtolower('attr_set') => $attr_set])->first();
                            // $atr = AttributeSet::where(["id" => $attr_set])->first();
                            // dd($atr);
                            if ($atr != null) {
                                $attr_set = $atr->id;
                            } else {
...
.image img{
  vertical-align:middle;
    ...
    public function renderNode($node, $prodCats)
    {
        $str = '';
