...
class PagesController extends Controller {
    ...
    public function index() {
        ...
        $todaysSales = Order::whereRaw("DATE(created_at) = '" . date('Y-m-d') . "'")
                ->whereNotIn("order_status", [0, 4, 6, 10])->where('prefix', $jsonString['prefix'])
                ->sum('pay_amt');
        ...
        $yearlyOrders = Order::whereRaw("YEAR(created_at) = '" . date('Y') . "'")
                ...
                ->count();
        $totalOrders = Order::whereNotIn("order_status", [0, 4, 6, 10])->where('prefix', $jsonString['prefix'])->count();
        $topProducts = HasProducts::with('product')->where('prefix', $jsonString['prefix'])->limit(5)->groupBy('prod_id')->orderBy('quantity', 'desc')->get(['prod_id', DB::raw('count(prod_id) as top'), DB::raw('sum(qty) as quantity')]);
        ...
        $topUsers = Order::whereNotIn("order_status", [0, 4, 6, 10])->where('prefix', $jsonString['prefix'])->with('users')->limit(10)->groupBy('user_id')->orderBy('total_amount', 'desc')->get(['user_id', DB::raw('count(user_id) as top'), DB::raw('sum(pay_amt) as total_amount')]);
        $latestOrders = Order::whereNotIn('order_status', [3, 4, 5, 6, 10])->where('prefix', $jsonString['prefix'])->limit(10)->orderBy('created_at', 'desc')->get();
        ...
        $salesGraph0 = Order::whereNotIn("order_status", [0, 4, 6, 10])->where('prefix', $jsonString['prefix'])->orderBy('created_at', 'asc')->where('created_at', '>=', date('Y-m-d', strtotime("-7 day")))->groupBy(DB::raw("DATE(created_at)"))->get(['created_at', DB::raw('sum(pay_amt) as total_amount')])->toArray();
        $orderGraph0 = Order::whereNotIn("order_status", [0, 4, 6, 10])->where('prefix', $jsonString['prefix'])->orderBy('created_at', 'asc')->where('created_at', '>=', date('Y-m-d', strtotime("-7 day")))->groupBy(DB::raw("DATE(created_at)"))->get(['created_at', DB::raw('count(id) as total_order')])->toArray();
