    ...
    $html .= '<div id="portfolio" class="portfolio  grid-container clearfix">';
        foreach($cats as $c)
        {
            ...
            foreach(App\Models\StoreTheme::where("cat_id",$catID)->where("status",1)->get() as $theme)
            {
                ...
                $html .= '<a type="button" href="' . route("admin.home.applyMerchantTheme", ["cateId" => $catID, "themeId" => $themeId]) . '" class="btn btn-block ">Appy Theme</a>';
                //$html .= '<a type="button" href="javascript:;" onClick="applyMerchantTheme('.$catID.','.$themeId.')" target="_blank" class="btn btn-block ">Appy Theme</a>';
                $html .= '<a type="button" href="'.$themeVal.'?theme='.$theme->name.'" target="_blank" class="btn btn-block marginauto btn-trans-whiteborder">View Theme</a>';
                $html .= '</div>';
                $html .= '</div>';
                $html .= '</div>';
                $html .= '</article>';
