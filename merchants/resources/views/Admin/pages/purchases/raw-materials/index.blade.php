        ...
        <div class="col-md-12">
             <div class="box">
                ...
                <div class="box-header box-tools filter-box col-md-9 noBorder rightBorder">
                    {!! Form::open(['method' => 'get', 'route' => 'admin.raw-material.view' , 'id' => 'searchForm' ]) !!}
                    ...
                    <div class="form-group col-md-4">
                        {!! Form::select('status',['1'=>'Enable', '0'=>'Disable'],Input::get('status'), ["class"=>'form-control ', "placeholder"=>"Status"]) !!}
                    </div>
                    <!-- <div class="form-group col-md-4">
                    ...
                    <div class="form-group col-md-4 noBottomMargin">
                        <div class=" button-filter-search col-md-6 no-padding">
                            <button type="submit" name="search" class="btn sbtn btn-primary form-control" style="margin-left: 0px;" value="Search"><i class="fa fa-search" aria-hidden="true"></i> Filter</button>
                        </div>
                        <div class="btn-group button-filter col-md-5 noBottomMargin no-padding">
                            <a href="{{route('admin.raw-material.view')}}"><button type="button" class="btn sbtn reset-btn btn-block" value="reset">Reset</button></a>
                        </div>
                    </div>
                    {!! Form::close() !!}
                </div>
                <div class="box-header col-md-3">
                    <a href="{!! route('admin.raw-material.add') !!}" class="btn btn-default pull-right col-md-12" type="button">Add Raw Material</a>
                </div>
...
<!--                                <td>{{$product->id }}</td>-->
                                <td>
                                    <!-- <img src="{{@$product->prodImage }}" class="admin-profile-picture" /> -->
                                    <div class="product-name">{{$product->product }}</div>
                                </td>

                                <td>{{$product->product_code }}</td>
                                <td>{{ isset($product->buying_unit_measure)?$product->buying_unit_measure->unit:''}} </td>
                        <td>{{ isset($product->consumption_unit_measure)? $product->consumption_unit_measure->unit:''}}</td>
                        <td>{{ $product->conversion }}</td>
                        <td>{{ $product->stock }}</td>
                        <td>{{ @$product->updatedBy->firstname }}</td>
                        <td>
                             @if($product->status==1)
                            <a href="{!! route('admin.raw-material.checkStatus',['id'=>$product->id]) !!}" class="" ui-toggle-class="" onclick="return confirm('Are you sure you want to disable this record?')" data-toggle="tooltip" title="Enabled"><i class="fa fa-check btn-plen btn"></i></a>
                            @elseif($product->status==0)
                            <a href="{!! route('admin.raw-material.checkStatus',['id'=>$product->id]) !!}" class="" ui-toggle-class="" onclick="return confirm('Are you sure you want to enable this record?')" data-toggle="tooltip" title="Disabled"><i class="fa fa-times btn-plen btn"></i></a>
                            @endif
                        </td>
                        <td>
                            <a href="{!! route('admin.raw-material.edit',['id'=>$product->id]) !!}"  class="" ui-toggle-class="" data-toggle="tooltip" title="Edit"><i class="fa fa-pencil-square-o fa-fw"></i></a>
<!--                            <a href="{!! route('admin.products.duplicate',['id'=>$product->id]) !!}" class="label label-info active" ui-toggle-class="" onclick="return confirm('Do you want to create duplicate product?')">Duplicate</a><br>-->
                            ...
                            <a href="{!! route('admin.raw-material.delete',['id'=>$product->id]) !!}" class="" ui-toggle-class="" onclick="return confirm('Are you sure you want to delete this record?')" data-toggle="tooltip" title="Delete"><i class="fa fa-trash fa-fw"></i></a>
