 ...
 * @return number
 */
function response1($params, $salt) {
    if (!is_array($params))
...
class Misc {
    ...
    public static function show_page($result) {
        ...
        if ($result['status'] === Misc::SUCCESS)
            echo '<script>window.location.href="' . $result['data'] . '"</script>'; //wp_redirect("http://celebrity-connect.cruxservers.in");
        else
...
function payu_success() {
    ...
    $url = "/inficart/payu-success?data=" . $postdata;
...
function payu_failure() {
    ...
    $postdata = json_encode($_POST);
   // $url =  "/inficart/payu-failure?data=" . $postdata;
    $url =  "/payu-failure?data=" . $postdata;
    echo "<script>window.location.href='" . $url . "'</script>";
}
